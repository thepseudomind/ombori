...
const Users = ({status} : IMain)=>{
    enum loadMoreUsers { inactive=0, loading=1, noMoreUsers = 2};
   ...
   // Render preloader upon page load (no products) or fetched Product list when available
    return (
        <div className="users" onScroll={handleScroll}>
            ...
            {loadingMoreUsers === loadMoreUsers[2] ? <NotificationBox message="No more users"/> : <span></span> /* Show notification when there's no more users */}
        </div>
    );
}
